...
test_that("test_dir() filter", {
  res <- test_dir(test_path("test_dir"), reporter = "silent", filter = "basic|empty")
  df <- as.data.frame(res)
  expect_identical(unique(df$file), c("test-basic.R", "test-empty.R"))
})

test_that("test_dir() helpers", {
  res <- test_dir(test_path("test_dir"), reporter = "silent", filter = "helper")
  df <- as.data.frame(res)
