...
* All `expect_*` give clearer error messages if you forget the `object`
  or `expected` arguments (#743).

* `expect_equal()` compares infinite values correctly (#789).

* In `expect_equal_to_reference()`, the default value for `update` has
  returned `FALSE` (@BrodieG, #683).

* `expect_error()` now returns the error object as documentated (#724).
  It now warns if you're using a custom expectation class and you're
...
* `JUnitReporter` no longer replaces `.` in class names (#753),  and
  creates ouput more compatibility with Jenkins (#806, @comicfans).

* `ProgressReporter`, the default reporter:

    * Now automatically generates a context from the file name and no longer
    ...
    * Now uses a 3 character wide column to display test successes, so up to
...
* `reporter$end_reporter()` is now only called when testing completes 
  successfully. This ensures that you don't get final reports when the test 
  fails partway through (#727).
...
* `expectation` objects now contain the failure message, even when succesful 
  (#836)
...
* New `testing_package()` which retrieves the name of the package currently 
  being tested (#699).
