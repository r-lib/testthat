...
#' Test reporter: verbose test progress
#'
#' This reporter lets you see detail of errors quickly.
#'
...
#'
#' @export
#' @export VeryVerboseReporter
#' @aliases VeryVerboseReporter
#' @keywords debugging
#' @param ... Arguments used to initialise class
VeryVerboseReporter <- setRefClass("VeryVerboseReporter", contains = "Reporter",
  fields = list(
    "failures" = "list",
    "n" = "integer",
    "has_tests" = "logical",
  ...
  methods = list(
    ...
    add_result = function(result) {
      ...
      if (n + 1 > length(labels) || n + 1 > max_reports) {
        ...
        failures[[n]] <<- result
        cat(colourise("Fail: ", fg = "red"))
        cat(colourise(result$test, fg = "red"))
        cat("\n")
        cat(colourise(result$failure_msg, fg = "red"))
        cat("\n")
      }
    },
