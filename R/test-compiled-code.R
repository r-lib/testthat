...
expect_cpp_tests_pass <- function(package) {
  ...
  tryCatch(
    ...
    error = function(e) {
      cli::cli_warn(
        "Failed to call test entrypoint {.str {run_testthat_tests}}."
      )
    }
  )
...
use_catch <- function(dir = getwd()) {
  ...
  if (!file.exists(desc_path)) {
    cli::cli_abort("No DESCRIPTION file at path {.file {desc_path}}.")
  }
  ...
  if (!nzchar(pkg)) {
    cli::cli_abort(
      "No {.field Package} field in DESCRIPTION file {.file {desc_path}}."
    )
  }

  src_dir <- file.path(dir, "src")
  if (!file.exists(src_dir) && !dir.create(src_dir)) {
    cli::cli_abort("Failed to create {.file src/} directory {.file {src_dir}}.")
  }
  ...
  if (!success) {
    cli::cli_abort(
      "Failed to copy {.file test-runner.cpp} to {.file {src_dir}}."
    )
  }
  ...
  if (!success) {
    cli::cli_abort(
      "Failed to copy {.file test-example.cpp} to {.file {src_dir}}."
    )
  }
  ...
  if (!file.exists(test_dir) && !dir.create(test_dir, recursive = TRUE)) {
    cli::cli_abort(
      "Failed to create {.file tests/testthat/} directory {.file {test_dir}}."
    )
  }
  ...
  cli::cli_inform(
    c(
      ...
      "i" = "Please ensure you have {.code useDynLib({pkg}, .registration = TRUE)} in your NAMESPACE."
    )
  )
}
...
get_routine <- function(package, routine) {
  ...
  cli::cli_abort(
    "Failed to locate routine {.str {routine}} in package {.pkg {package}}."
  )
}
