...
We recommend organising your test files so that there's a one-to-one correspondence between the files in `R/` and the files in `tests/testthat/` so that (e.g.) `R/file-one.R` has a matching `tests/testthat/test-file-one.R`.
This correspondence is maintained by functions like `usethis::use_r()` and `usethis::use_test()` and is respected by functions like `devtools::test_active_file()` and `devtools::test_coverage_active_file()`.
