...
This function should start with `skip_` and live in a `test/helper-{something}.R`:
...
But `testthat::skip` is only ever run when you're inside a test, where you can assume that testthat is installed.
...
On the other hand, it mixed code and tests in an unusual way, and it when you're focused on the tests, the existence of a skip in code is easy to miss.
