...
-   `skip_if_not_installed()` skips if a required package is not installed. You can optional supply a minimal version too.
...
-   `skip_on_covr()` skips test during code coverage.
...
# Only run test if R has memory profiling capabililties
...
Another useful technique is to build a `skip()` directly into a package function.
