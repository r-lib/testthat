...
Each `local_*()` function has a companion `with_()` function, which is a nod to `with()`, and the inspiration for withr's name. We won't use the `with_*()` functions here, but you can learn more about them at [withr.r-lib.org](http://withr.r-lib.org).
...
(If we wrote this today, would call it `local_create_package()` to emphasise the local-ness of it.)
...
So far we have applied our test fixture to individual tests, but it's also possible to apply them to an entire file, just by calling the test fixture at the top of the file. This has both advantages and disadvantages:
...
Generally, I think it's to copy and paste test fixtures across many tests --- sure, it adds some duplication to your code, but it makes debugging test failures so much easier.
