...
-   `expect_equivalent()` has been deprecated since it is similar to `expect_equal(ignore_attr = TRUE)` -- though note that
    `expect_equivalent()` also ignored differences in object names [(#1241)](https://github.com/r-lib/testthat/issues/1241).
