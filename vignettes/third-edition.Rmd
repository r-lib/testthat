...
-   `expect_equal()` and `expect_identical()` now use the [**waldo**](https://waldo.r-lib.org/) package instead of `identical()` and `all.equal()`. This makes them more consistent and provides an enhanced display of differences when a test fails.
...
-   `setup()` an `teardown()` are deprecated in favour of test fixtures. See `vignette("test-fixtures")` for details.
