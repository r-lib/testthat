...
By default, parallel testthat will use 2 cores.
To customise that value for your local computer we recommend adding an entry for `TESTTHAT_CPUS` to your `.Rprofile`.
The easiest way to do that is use `usethis::edit_r_profile()` and then add something like the following:
